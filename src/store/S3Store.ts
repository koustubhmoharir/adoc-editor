...
async function scanLocalFiles(rootNode: DirectoryNodeModel, s3Prefix: string): Promise<Map<string, LocalFileRecord>> {
...
export class S3Store {
    ...
    async sync(rootNode: DirectoryNodeModel) {
        ...
        const localFiles = await scanLocalFiles(rootNode, this.settings.prefix);
        ...
        const remoteRecordsByPath = await fetchRemoteRecords(s3Client, this.settings.prefix, this.settings.bucket, metaCacheDir, baseRecordsByPath);

        // 6. Compute Actions
        const statusItems = await matchRecords(baseRecordsByPath, localFiles, remoteRecordsByPath, this.settings.prefix);
        runInAction(() => {
