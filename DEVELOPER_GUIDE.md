...
- **Example**:
  ...
  ```
- **Usage in Tests**:
  - Use reusable helpers from `tests/helpers/` instead of accessing globals directly.
    - `waitForTestGlobals(page)`: Waits for globals to be available.
    - `getEditorContent(page)`: Safely retrieves editor content.
    - `setMockPickerConfig(page, config)`: Sets up file system mocks.
  - If a specific helper doesn't exist, create a new helper following the same pattern as the existing helpers.
